...
# (正则, 说明)：命中即阻断，要求人工确认
DANGEROUS = [
    # 危险的是「字面的 / ~ * . .. $VAR」本身，不是以它们开头的路径。
    # 所以规则拆成两段：根 + 尾部清扫。
    #   根：  /  ~  .  ..  $VAR  *      （`/\*?` 一并覆盖 `/*`）
    #   尾部：( / 、 /* 、 /.* )        —— 表示「这个根下面的一切」
    # 尾部之后必须紧跟分隔符才算命中，否则 `rm -rf /tmp/build`、`rm -rf ~/cache`、
    # `rm -rf ./dist` 这类正常清理会被拦下。原版 (r"…\s+(/|~|\*|$)") 未锚定、
    # 只看目标首字符就命中，正是这个毛病。
    #
    # 目标集合要覆盖全，否则会漏。踩过两次：
    #   1. 只列 / ~ * 时，`rm -fr ../..`、`rm -fr $HOME` 全部放行；
    #   2. 补上 .{1,2}(/\.{2})* 与 $VAR 之后，仍漏 `$HOME/*`、`${HOME}/*`
    #      ——它们和 `~/*` 语义完全一样，却一个拦一个放；以及 `~/.*`
    #      ——清空家目录下全部点文件，含 .ssh。现在由尾部统一覆盖。
    #
    # `~*` 故意不拦：实测 bash 不对它做家目录展开（`~*` 只会 glob 当前目录里
    # 以 ~ 开头的文件名），它不是家目录，拦了属于误伤。
    # 两侧可选引号 —— `rm -rf "$HOME"` 同样要拦。
    # 旗标写法 -rf / -fr / -r -f / --recursive --force 一律覆盖。
    (r"\brm(?:\s+-[a-zA-Z-]+)*\s+[\"']?"
     r"(?:/\*?"                                  # / 或 /*
     r"|~"                                       # ~
     r"|\.{1,2}(?:/\.{2})*"                      # . .. ../.. ../../..
     ...
     r"(?:/(?:\.?\*)?)?"                         # 尾部：/ 、 /* 、 /.*
     r"[\"']?(\s|$|[;&|)])",
     "递归强制删除根目录/家目录/当前目录全部内容"),
    (r"\brm\s+-rf\s+[^\s]*\s*;/", "删除后接绝对路径，风险极高"),
    (r"\bmkfs(\.[a-z0-9]+)?\b", "格式化文件系统"),
    (r"\bdd\s+if=", "dd 直接写设备"),
    (r":\(\)\s*\{\s*:\|:&\s*\}\s*;", "fork 炸弹"),
    # --force-with-lease 是官方推荐的安全强推（远端有新提交时会拒绝），
    # 原版把它一起拦下、提示还写"覆盖远端历史"，是误伤 + 说明错误，已移除。
    # (?![-\w]) 是必需的：否则 --force 会前缀匹配掉 --force-with-lease / --force-if-includes。
    # [^\n;&|]*? 让 -f 可以出现在 push 之后任意位置（如 git push origin main -f），
    # 同时不跨越 ; & |，避免把 `git push; ls -f` 也算成强推。
    (r"\bgit\s+push\b[^\n;&|]*?(?:\s-f\b|\s--force(?![-\w]))",
     "强制推送（覆盖远端历史）"),

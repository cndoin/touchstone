...
DANGEROUS = [
    ...
    # 必须要求目标后紧跟分隔符，否则 `rm -rf /tmp/build`、`rm -rf ~/cache`、
    # `rm -rf ./dist` 这类正常清理会被拦下（原版 (r"…\s+(/|~|\*|$)") 未锚定，
    # 以及 r"\brm\s+-fr\b" 只看旗标顺序不看目标，都是这个问题）。
    #
    # 目标集合要覆盖全，否则会漏：曾一度只列 / ~ *，结果 `rm -fr ../..`、
    # `rm -fr $HOME` 这类目标不以三者开头的写法全部放行。现在补上：
    #   .{1,2}(/..)*      —— . / ./ / ./* / .. / ../ / ../.. / ../* / ../../*
    #   $VAR / ${VAR}     —— 静态求不出值，宁可要求确认

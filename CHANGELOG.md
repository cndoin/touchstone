...
v4.1.0 的部署无需迁移（不想要新层可不改 `description`，但会失去数学幻觉防护）。

### 背景（为什么加）

v4.1.0 解决的是「小任务不该加载」；但**数学计算这一类的幻觉一直没被专门覆盖**。
它跟一般事实幻觉有个本质区别：**至少一半是可判定的**——算术对不对、证明合不合法，机器能验。
把它当普通事实幻觉处理，等于放弃了自己最强的那一半武器。

调研后确认错误分四层，且**治法互不通用**（详见新文件）：

| 层 | 症状 | 治法 | 现状 |
|---|---|---|---|
| ① 算术 / 符号算错 | `3×7=20`、移项漏变号 | 强制确定性引擎 | 已解决 |
| ② 可执行但未接地 | 公式对、量纲对，**变量代错** | 双层校验 | 研究前沿 |
| ③ 题目病态 / 缺条件 | 少一个条件、自相矛盾 | 拒答 / 澄清闸门 | **最难** |
| ④ 验证器本身不可信 | 规则式漏判；模型式被刷分 | 去锚定 | 有明确解法 |

### 新增：数学模式（M7）

`SKILL.md` 新增 `## 数学模式` 章节，给出四条硬条款：

1. **数值结论必须有执行记录**——来自实跑代码，不许来自「我认为」；
2. **工具成功 ≠ 算对了**——跑通只证明语法对，第二层必须问「这个变量指题面里的哪个词」；
3. **答题前判条件是否齐**——缺则先追问；不可追问则条件化；禁止假设填空；
4. **核对答案时裁判先自立作答**——铁律 2 的数学形态，顺序反了整条链归零。

新增 `references/18-math-mode.md`（完整操作手册，含最小执行清单与引用分档）。

### 新增：description 第 (6) 条触发

`description` 从 5 条触发扩到 6 条，补上**"输出包含会被采用的数值或符号结论"**
（算钱/税率/工期/容量/比例、财务或统计口径、公式推导、工程参数、复现论文数字）。
「不适用」清单同步补上"单点算术问答"，避免"3 的平方是多少"这类问题误加载。

### 关键发现：第③层不能靠"多想一会儿"

Soohak 基准（arXiv 2605.09063）的 refusal 子集上**没有任何模型超过 50%**，
且**拒答能力不随模型规模、也不随测试时计算扩展**；推理时 thinking mode 在未经训练的模型上
反而**降低**主动批判性思考。故本版明确**禁止**用"再仔细检查一遍"当数学闸门，改用结构化前置三问。

### 顺带修复：测试用例数文字漂移

上游 PR 合并后 `selftest.py` 实际为 **72** 条，但 README（中/英）、SKILL.md（中/英）
共 6 处仍写 38。本版一并更正为 72（`robustness` 70、`stability` 99/97 未变）。

### 改动清单

| 文件 | 改动 |
|---|---|
| `SKILL.md` | 新增「数学模式」章节 + 3 条反模式 + description 第 (6) 条 + 参考索引 + 版本号 |
| `references/18-math-mode.md` | **新增文件**：四层错误与治法、确定性引擎、双层校验、病态题闸门、去锚定、引用分档 |
| `i18n/en/SKILL.md` | 英文镜像同步（Math mode 章节 + description + 索引 + 版本号） |
| `references/13-frontier-2026.md` | 新增 §6 数学幻觉专项（11 篇论文，全部回溯 arXiv 摘要原文）+ §6.1 取舍说明 |
| `references/06-cost-routing.md` | 新增数学档：确定性引擎排在选检测器之前；路由表补 2 行 |
| `references/09-anti-patterns.md` | 新增 5 条数学反模式 + 「数学层」自检清单 8 项 |
| `references/04-verification-workflow.md` | 流程块新增 `[M] 数学闸` |
| `README.md` / `README.en.md` | 新增「数学模式」章节；核心机制七层→八层（加 M7）；风险分级加 L1+；用例数 38→72 |
| `CHANGELOG.md` | 本条 |
| `VERSION` / `CITATION.cff` | 4.1.0 → 4.2.0（六处同步） |

### 预期效果

- 含数值/符号结论的输出，**每个数字可追溯到执行记录**，"心算出错"这一类接近消除；
- 抓到「工具跑通了但变量代错」这一最隐蔽的失败模式（语法/量纲全过）；
- 病态题不再被硬答，改为追问或条件化，且**不依赖换更强模型**；
- 测试数与文档一致，不再有文字漂移。

---

## v4.1.0（2026-09-17）· 入场门：小任务不再启动全套核查

**MINOR：触发行为变了。** 路径、脚本接口、退出码均未改动，v4.0.0 的部署无需迁移。

### 背景（为什么改）

v4.0.0 的 `description` 写得太宽（"写报告、查资料、给建议、引用文献/API/论文、调用工具…"），
导致模型**对任何请求都会加载本套件**——包括改一个 CSS 值、重命名变量、解释一段代码这类小任务。
后果有两个：**① 响应变慢；② 烧掉大量与收益无关的 token**。
而这类任务既无可判定事实、又可逆、又不对外，全套核查的收益严格为 0。

### 新增：入场门（M0）

`SKILL.md` 新增 `## 入场门` 章节（位于所有章节之首），给出**四问清单 G1–G4**：

| # | 判据 |
|---|---|
| G1 | 改动是否 ≤1 个文件、答案为单点、无需外部资料 |
| G2 | 是否完全不涉及外部可判定事实（URL/DOI/论文/法条/API/版本/包名/路径/命令输出） |
| G3 | 是否可逆、不进版本库历史 |
| G4 | 是否不对外发布、不进入他人决策链 |

**四条全「是」→ 浅模式：不加载套件，直接答。** 任一条为「否」→ 入场。
并明确列出**必须升级为入场的例外**（用户点名核实 / 输出被直接采用 / 不可逆操作 / 用户先表态 / 自己想说"应该是"）。

### 新增：浅模式底线条款

入场门不加载套件，但**红线与不确定性降级仍然生效**——它们是「习惯」而非「流程」，成本≈0。
避免"不加载 = 无底线"的误读。

### 改动清单

- `SKILL.md`：`description` 重写为"仅在满足 5 类条件时使用 + 明确不适用场景"；新增 `## 入场门`；风险分级 L0 行指向入场门
- `i18n/en/SKILL.md`：同步上述全部改动；顺带补齐此前英文版缺失的 **Q0 模型档位**（标题同步改为 Four questions）
- `README.md` / `README.en.md`：新增「什么时候**不要**用」章节；核心机制表补 **M0 入场门**（六层 → 七层）；风险分级 L0 行同步
- `README.en.md`：顺带补齐英文快速开始中缺失的 `model_profile.py` 步骤（与中文版对齐）
- `references/06-cost-routing.md`：新增 §0「是否入场（最省的一档：不核查）」，路由表首行补「零成本」档
- `references/09-anti-patterns.md`：反模式表新增「对小改动也启动全套核查」；交付前自检表新增入场门检查项
- `references/02-context-settings.md`：边界情况表新增「小改动/单点问题/闲聊创意 → 不进入本套件」
- `references/08-code-mode.md`：§5 工作流前新增入场门前置说明
- `references/04-verification-workflow.md`：完整流程新增 `[门] 入场门` 步骤

### 预期效果

日常小任务的加载开销从"整套 8 步流程"降为"0 次加载"。大型项目、高风险场景、
含外部可判定事实的输出**不受影响**——这些场景仍 100% 入场。
